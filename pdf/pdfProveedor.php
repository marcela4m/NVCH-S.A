...
require_once('class.ezpdf.php');
	...
	$resSql=mysql_query("select * from proveedor",$con) or die("<br>Error consulta</br>".mysql_error());
	
$pdf = new Cezpdf('A4');
$pdf->selectFont('fonts/Helvetica.afm');
	
	while($row=mysql_fetch_row($resSql)){
		$data[]=array('ID_PROVEEDOR'=>$row[0], 'EMPRESA'=>$row[1],'DIRECCION'=>$row[2],'TELEFONO'=>$row[3]);
	}
