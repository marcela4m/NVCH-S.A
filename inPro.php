...
<br><br><br>
					...
					<article class="grid_6">
						<div class="indent">
							<h2 class="indent-bot">NUEVO PROVEEDOR</h2>
...
mysql_select_db('u893654268_3')or die ('Error al seleccionar la Base de Datos: '.mysql_error());
	...
	if($nom==NULL&&$dir==NULL&&$tel==NULL) {
	}else if($nom==NULL|$dir==NULL|$tel==NULL) {
		
echo "<h2> Uno o mas campos estan vacios.</h2>";
		
	}else{
			// Comprobamos si el nombre de usuario o la cuenta de correo ya existían
			$checkuser = mysql_query("SELECT * FROM proveedor WHERE empresa='$nom'");
			...
			if ($username_exist>0) {
				echo "<h2>El proveedor ingresado ya existe</h2>";
				
			}else{
				$query = 'INSERT INTO proveedor ( empresa, addressP, phoneP)
				VALUES (\''.$nom.'\',\''.$dir.'\',\''.$tel.'\')';
				mysql_query($query) or die(mysql_error());
				echo 'El proveedor '.$nom.' ha sido registrado de manera satisfactoria.<br />';
