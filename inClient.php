...
<br><br><br>
					...
					<article class="grid_6">
						<div class="indent">
							<h2 class="indent-bot">NUEVO CLIENTE</h2>

							<ul class="list-2">
								

								
								<form action="inClient.php" method="post">
...
mysql_select_db('u893654268_3')or die ('Error al seleccionar la Base de Datos: '.mysql_error());
	...
	if($cedula==NULL&&$nom==NULL&&$dir==NULL&&$tel==NULL) {
	}else if($cedula==NULL|$nom==NULL|$dir==NULL|$tel==NULL) {
...
echo "<h2> Uno o mas campos estan vacios.</h2>";
			...
			if ($username_exist>0) {
				echo "<h2>El usuario ingresado ya existe</h2>";
				
			}else{
				$query = 'INSERT INTO clientes (CC, nameC, addressC, phoneC)
				VALUES (\''.$cedula.'\',\''.$nom.'\',\''.$dir.'\',\''.$tel.'\')';
